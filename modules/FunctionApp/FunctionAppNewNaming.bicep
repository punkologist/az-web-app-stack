...
resource appInsights 'Microsoft.Insights/components@2020-02-02-preview' =  [ for i in range(0, length(functionApps)) : {
  name:'${functionApps[i].appName}-ai'
   tags:tags
  location: location
  kind: 'web'
  properties: {
...
resource functionApp 'Microsoft.Web/sites@2020-06-01' = [ for i in range(0, length(functionApps)) : {
  ...
  properties: {
    ...
    hostNameSslStates: [
      {
        name: '${functionApps[i].appNameShort}.azurewebsites.net'
        sslState: 'Disabled'
        hostType: 'Standard'
      }
      {
        name: '${functionApps[i].appNameShort}.scm.azurewebsites.net'
        sslState: 'Disabled'
        hostType: 'Standard'
      }
    ]
