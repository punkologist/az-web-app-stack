...
var httpListeners = [for (app,i) in appsArray:{
  name: 'listenerHttp-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}'
  properties: {
...
var httpsListeners = [for (app,i) in appsArray:{
  name: 'listenerHttps-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}'
  properties: {
    frontendIPConfiguration: {
      id: app.private ? appGwPrivateFrontendIpConfigId : appGwFrontendIpConfigId 
    }
...
var httpRoutingRules = [for (app,i) in appsArray:{
  name: 'rrHttp-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}'
  properties: {
    priority: i+1
    ruleType: 'Basic'
    httpListener: {
      id: resourceId('Microsoft.Network/applicationGateways/httpListeners/', applicationGatewayName, 'listenerHttp-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}')
    }
    redirectConfiguration: {
      id:resourceId('Microsoft.Network/applicationGateways/redirectConfigurations/', applicationGatewayName, 'rdc-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}')  
    }
...
var httpsRoutingRules = [for (app,i) in appsArray:{
  name: 'rrHttps-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}'
  properties: {
    priority: i+100
    ruleType: 'Basic'
    httpListener: {
      id: resourceId('Microsoft.Network/applicationGateways/httpListeners/', applicationGatewayName, 'listenerHttps-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}')
    }
    backendAddressPool: {
      id: resourceId('Microsoft.Network/applicationGateways/backendAddressPools/', applicationGatewayName, 'beap-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}')
    }
    backendHttpSettings: {
...
var webbackendPools = [for (app,i) in appsArray:{
  name: 'beap-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}'
  properties: {
    backendAddresses: [
      {
...
var redirectConfigurations = [for (app,i) in appsArray:{
      name: 'rdc-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}'
      properties: {
          ...
          requestRoutingRules: [
              {
                  id: resourceId('Microsoft.Network/applicationGateways/requestRoutingRules/', applicationGatewayName, 'rrHttp-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}')
              }
          ]
      }
  }]
  ...
  var httpWwwRedirectsListeners = [for (app,i) in wwwRedirects:{
    name: 'listenerHttpWww-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}'
    properties: {
      frontendIPConfiguration: {
        id: app.private ? appGwPrivateFrontendIpConfigId : appGwFrontendIpConfigId 
      }
  ...
  var httpsWwwRedirectsListeners = [for (app,i) in wwwRedirects:{
    name: 'listenerHttpsWww-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}'
    properties: {
      frontendIPConfiguration: {
        id: app.private ? appGwPrivateFrontendIpConfigId : appGwFrontendIpConfigId 
      }
  ...
  var httpWwwRedirectsRoutingRules = [for (app,i) in wwwRedirects:{
    name: 'rrHttpWww-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}'
    properties: {
      priority: i+400
      ruleType: 'Basic'
      httpListener: {
        id: resourceId('Microsoft.Network/applicationGateways/httpListeners/', applicationGatewayName, 'listenerHttpWww-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}')
      }
      redirectConfiguration: {
        id:resourceId('Microsoft.Network/applicationGateways/redirectConfigurations/', applicationGatewayName, 'rdcWwwHttp-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}')  
      }
  ...
  var httpsWwwRedirectsRoutingRules = [for (app,i) in wwwRedirects:{
    name: 'rrHttpsWww-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}'
    properties: {
      priority: i+500
      ruleType: 'Basic'
      httpListener: {
        id: resourceId('Microsoft.Network/applicationGateways/httpListeners/', applicationGatewayName, 'listenerHttpsWww-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}')
      }
      redirectConfiguration: {
        id:resourceId('Microsoft.Network/applicationGateways/redirectConfigurations/', applicationGatewayName, 'rdcWwwHttps-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}')  
      }
  ...
  var redirectWwwRedirectsHttpConfigurations = [for (app,i) in wwwRedirects:{
    name: 'rdcWwwHttp-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}'
    properties: {
        redirectType: 'Permanent'
        targetListener: {
          id: resourceId('Microsoft.Network/applicationGateways/httpListeners/', applicationGatewayName, 'listenerHttps-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}')
        ...
        requestRoutingRules: [
            {
                id: resourceId('Microsoft.Network/applicationGateways/requestRoutingRules/', applicationGatewayName, 'rrHttpWww-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}')
            }
        ]
    }
}]
...
var redirectWwwRedirectsHttpsConfigurations = [for (app,i) in wwwRedirects:{
  name: 'rdcWwwHttps-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}'
  properties: {
      redirectType: 'Permanent'
      targetListener: {
        id: resourceId('Microsoft.Network/applicationGateways/httpListeners/', applicationGatewayName, 'listenerHttps-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}')
      ...
      requestRoutingRules: [
          {
              id: resourceId('Microsoft.Network/applicationGateways/requestRoutingRules/', applicationGatewayName, 'rrHttpsWww-${app.appNamePrefix}-${uniqueString(resourceGroup().id)}')
          }
      ]
  }
}]
  ...
  var customBackendSettings = [for (app,i) in filteredCustomDomainApps:{
    ...
    properties: {
      ...
      probe: {
        id: resourceId('Microsoft.Network/applicationGateways/probes/', applicationGatewayName, '${app.appNamePrefix}-probe')
      }
    }
  }]
